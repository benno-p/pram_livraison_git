		    ...
		    <div class="row mt-4">
		        <div class="col-12">
		            <div class="card mb-4 pb-4">
		                ...
		                <div class="card-body">
		                	<form method="POST" action="" v-on:submit.prevent="valide_mare()">
...
export default{
   ...
   //          }
            ...
            formData.append('mare_id', this.$route.params.id ? this.$route.params.id : '');
            formData.append('caracterisation_id', this.caracterisation ? this.caracterisation : '');
            formData.append('validation_type', 'mare');

            axios.post(base_url + '/validation_mares', formData, { 
                _method: 'POST',
            })
            .then(function (response) {
                if(response.data.error == true){
                    ...
                    self.$router.push({path: '/validation_mares'});
                }
            })
    ...
    mounted:function(){
        const self = this;
        self.isLoading = true;
        axios.get(base_url + '/validation_mares/'+self.$route.params.id+'/edit')
        .then(function (resp) {
            self.all_data = resp.data;
            self.caracterisations = resp.data.caracterisations;
            self.statuts = resp.data.statuts;

            if(resp.data.mare && resp.data.mare.statut && resp.data.mare.statut.id){
            	self.statut = self.statuts.find(s => s.id === resp.data.mare.statut.id);
            }
