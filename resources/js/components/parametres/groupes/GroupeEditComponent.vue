    ...
    export default {
        ...
        methods: {
            ...
            create:function(){
                ...
                axios.post(base_url + '/groupes/' + self.groupe.id, formData, { 
                    _method: 'POST',
                })
        ...
        mounted(){
            const self = this;
            axios.get(base_url + '/groupes/'+ this.$route.params.id + '/edit')
            .then(function (resp) {
                self.isLoading = true;
                self.groupe = resp.data.groupe;
                ...
                if(self.groupe.departements.length > 0){
                    self.types_selected = self.types.find(t => t.nom_interne === 'departements');
                    self.groupe.departements.forEach(function(dep){
                        self.departements_selected.push(dep);
                    })
                }else if(self.groupe.intercommunalites.length > 0){
                    self.types_selected = self.types.find(t => t.nom_interne === 'intercommunalites');
                    self.groupe.intercommunalites.forEach(function(interco){
                        self.intercommunalites_selected.push(interco);
                    })
                }else if(self.groupe.communes.length > 0){
                    self.types_selected = self.types.find(t => t.nom_interne === 'communes');
                    self.groupe.communes.forEach(function(com){
                        self.communes_selected.push(com);
                    })
                }else{
                    self.types_selected = self.types.find(t => t.nom_interne === 'groupe')
                }
