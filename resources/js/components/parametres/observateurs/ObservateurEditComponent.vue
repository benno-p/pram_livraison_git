    ...
    export default {
        ...
        methods: {
            ...
            create:function(){
                ...
                axios.post(base_url + '/observateurs/'+self.observateur.id, formData, { 
                    _method: 'POST',
                })
        ...
        mounted(){
            const self = this;
            axios.get(base_url + '/observateurs/'+ this.$route.params.id + '/edit')
            .then(function (resp) {
                self.isLoading = true;
                self.observateur = resp.data.observateur;
                self.nom = self.observateur.nom;
                self.prenom = self.observateur.prenom;
                self.organisme = self.observateur.organisme;
                self.nom_source = self.observateur.nom_source;
